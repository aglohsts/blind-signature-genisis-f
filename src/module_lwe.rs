...
impl ModuleLweEncoding {
    ...
    /// Returns the number of encoded bits `t`.
    pub fn bits(&self) -> i64 {
        self.t
    }

    /// Returns the length `ell_xi` of the function randomness.
    pub fn randomness_length(&self) -> i64 {
        self.ell_xi
    }

    /// Returns the bound `B_xi = psi_xi * sqrt(ell_xi * d)` as its
    /// square, matching the shape of `CommitmentKey`.
    pub fn randomness_bound_sqrd(&self) -> Z {
        let degree = self.modulus.get_degree();
        Z::from(self.psi_xi * self.psi_xi * self.ell_xi * degree)
    }

    /// Returns the encoding matrix `G` in its coefficient embedding.
    /// The LaZer statement builder reads it column by column.
    pub fn encoding_matrix(&self) -> &MatZq {
        &self.g_mat
    }

    /// Returns the binary decomposition of `input - 1` as a `t x 1`
    /// integer matrix. This is the binary part of the witness of the
    /// final-signature relation.
    pub fn encode(&self, input: &Z) -> MatZ {
    ...
    /// Evaluates the encoding term `G * enc(mu)` on its own. The LaZer
    /// statement uses this term separately from the masking term.
...
impl PublicFunction for ModuleLweEncoding {
    ...
    /// `f(kappa, mu, xi) = kappa * xi + G * enc(mu)`.
    fn eval(
        &self,
...
mod tests {
    ...
    /// Each of the three arguments changes the output on its own.
    #[test]
    ...
    /// The function stays linear in the randomness, which is the
    /// property the LaZer statement relies on.
    #[test]
