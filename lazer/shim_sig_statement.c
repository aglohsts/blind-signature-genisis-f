...
static int64_t center_mod_sig_q (int64_t value) {
...
static void set_constant_coefficient (poly_ptr poly, int64_t value) {
    poly_set_zero (poly);
    int_set_i64 (poly_get_coeff (poly, 0), center_mod_sig_q (value));
}

static void set_coefficient_vector (poly_ptr poly, const int64_t *values) {
    unsigned int i;
...
static void set_rotated_automorphism (poly_ptr poly, const int64_t *values,
                                      unsigned int shift) {
    unsigned int source;
    ...
    for (source = 0; source < BS_LAZER_SIG_D64_DEGREE; source++) {
        unsigned int exponent
            = source == 0 ? 0 : BS_LAZER_SIG_D64_DEGREE - source;
...
static void set_identity_entry (polymat_t matrix, unsigned int row,
                                unsigned int column) {
    poly_ptr entry = polymat_get_elem (matrix, row, column);
    int_set_i64 (poly_get_coeff (entry, 0), 1);
}

// select `rows` consecutive bounded columns starting at `first`
static void set_selector (polymat_t matrix, unsigned int rows,
                          unsigned int first) {
    unsigned int row;
...
void bs_sig_d64_statement_init (bs_sig_d64_statement *statement,
    ...
    const unsigned int h_plain = 2u * (BS_LAZER_SIG_D64_BOUNDED_COLUMNS + 1u);
    const unsigned int u_auto
        = 2u * (BS_LAZER_SIG_D64_BOUNDED_COLUMNS + 2u) + 1u;
    const unsigned int tag_auto = 2u * BS_LAZER_SIG_D64_BOUNDED_COLUMNS + 1u;
    ...
    for (row = 0; row < BS_LAZER_SIG_D64_DEGREE; row++) {
        ...
        set_coefficient_vector (entry,
                                tag_matrix + row * BS_LAZER_SIG_D64_TAG_BITS);
    ...
    for (row = 1; row < BS_LAZER_SIG_D64_DEGREE; row++) {
        ...
        poly_ptr entry
            = spolyvec_insert_elem (statement->linear[index], h_auto);
        poly_set_zero (entry);
    ...
    for (column = 0; column < BS_LAZER_SIG_D64_PREIMAGE_COLUMNS; column++)
        set_constant_coefficient (
            ...
            -1);
    statement
        ->quadratic_ptrs[BS_SIG_D64_EVAL_OFFSET + BS_SIG_D64_NORM_EQUATION]
    ...
    statement
        ->quadratic_ptrs[BS_SIG_D64_EVAL_OFFSET + BS_SIG_D64_INVERSE_EQUATION]
        = statement->quadratic[1];
    set_constant_coefficient (statement->constant[BS_SIG_D64_INVERSE_EQUATION],
                              -1);
