...
static uint64_t preimage_norm_sq (const int64_t *witness) {
    uint64_t norm = 0;
    size_t i;
...
static uint64_t mod_inverse (uint64_t value) {
    ...
    while (exponent > 0) {
        if ((exponent & 1u) != 0)
            result
                = (uint64_t)((unsigned __int128)result * base % BS_SIG_D64_Q);
...
// accumulate one bounded block and check its infinity bound
static int block_norm_sq (const int64_t *witness, size_t first, size_t count,
...
int bs_lazer_sig_d64_witness_is_valid (const int64_t *witness,
    ...
    return preimage_sq > 0 && preimage_sq <= BS_LAZER_SIG_D64_PREIMAGE_BOUND_SQ
        && function_randomness_sq
